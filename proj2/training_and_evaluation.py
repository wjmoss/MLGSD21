...
def train_model(model: nn.Module, dataset: Dataset, batch_size: int, loss_function: Callable, optimizer: Optimizer,
    ...
    for epoch in range(epochs):
        ...
        for x,y in tqdm(train_loader, total=num_train_batches):
            ...
            loss.backward()
            optimizer.step()
            accuracy = torch.sum(logits.argmax(dim=1) == y) / batch_size
            losses.append(loss)
...
def predict_model(model: nn.Module, dataset: Dataset, batch_size: int, attack_function: Union[Callable, None] = None,
    ...
    for x, y in tqdm(test_loader, total=num_batches):
        ...
        if attack_function is None:
          ...
          x_perm = attack_function(logits, x, y, **attack_args)
          #model.zero_grad()
          predictions.append(model(x_perm).argmax(dim=1))
        targets.append(y)
        ##########################################################
...
def evaluate_robustness_smoothing(base_classifier: nn.Module, sigma: float, dataset: Dataset,
    ...
    for x, y in tqdm(test_loader, total=len(dataset)):
        ##########################################################
        # YOUR CODE HERE
        cl, radius = model.certify(inputs=x, n0=num_samples_1, num_samples=num_samples_2, alpha=alpha, batch_size=certification_batch_size)
        if cl != -1:
          false_predictions += (cl != y)
          correct_certified += (cl == y)
          radii.append(radius)
        else:
          abstains += 1
        ##########################################################

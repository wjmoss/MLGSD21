...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        num_class_A = class_counts[top_class]
        
        # alpha level lower bound for p_A
        p_A_lower_bound = lower_confidence_bound(num_class_A=num_class_A, num_samples=num_samples, alpha=alpha)
        ##########################################################
        ...
        if p_A_lower_bound < 0.5:
            ...
            radius = self.sigma * normal.icdf(torch.tensor(p_A_lower_bound))
            ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        # YOUR CODE HERE
        top_class = class_counts.argmax()
        top2 = torch.topk(class_counts, 2)[0]
        if binom_test(top2[0], torch.sum(top2), p=0.5) < alpha:
          return top_class
        else:
          #SmoothClassifier.ABSTAIN
          return -1
        ##########################################################
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                #x += torch.normal(mean=torch.zeros(size=x.shape), std=self.sigma)
                probs = self.forward(x)
                this_batch_classes = probs.argmax(dim=1)
                for cl in this_batch_classes:
                  class_counts[cl] += 1
